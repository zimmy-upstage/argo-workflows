...
func TestOperateWorkflowPanicRecover(t *testing.T) {
	...
	controller.primaryProfile().kubernetesClient = nil
	wf := wfv1.MustUnmarshalWorkflow(helloWorldWf)
	ctx := context.Background()
	_, err := controller.primaryProfile().workflowClient.ArgoprojV1alpha1().Workflows("").Create(ctx, wf, metav1.CreateOptions{})
...
func Test_wfOperationCtx_reapplyUpdate(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		assert.NoError(t, controller.hydrator.Hydrate(common.PrimaryCluster(), wf))
		nodes := wfv1.Nodes{"foo": wfv1.NodeStatus{Name: "my-foo", Phase: wfv1.NodeSucceeded}}

		// now force a re-apply update
		updatedWf, err := woc.reapplyUpdate(ctx, controller.primaryProfile().workflowClient.ArgoprojV1alpha1().Workflows(""), nodes)
	...
	t.Run("ErrUpdatingCompletedWorkflow", func(t *testing.T) {
		...
		defer cancel()
		woc := newWorkflowOperationCtx(wf, controller)
		_, err := woc.reapplyUpdate(ctx, controller.primaryProfile().workflowClient.ArgoprojV1alpha1().Workflows(""), wfv1.Nodes{})
	...
	t.Run("ErrUpdatingCompletedNode", func(t *testing.T) {
		...
		defer cancel()
		woc := newWorkflowOperationCtx(wf, controller)
		_, err := woc.reapplyUpdate(ctx, controller.primaryProfile().workflowClient.ArgoprojV1alpha1().Workflows(""), wf.Status.Nodes)
...
func TestParamSubstitutionWithArtifact(t *testing.T) {
	...
	ctx := context.Background()
	woc.operate(ctx)
	wf, err := woc.controller.primaryProfile().workflowClient.ArgoprojV1alpha1().Workflows("").Get(ctx, wf.ObjectMeta.Name, metav1.GetOptions{})
	assert.NoError(t, err)
...
func TestResolvePodNameInRetries(t *testing.T) {
	...
	for _, tt := range tests {
		...
		woc.operate(ctx)
		assert.Equal(t, wfv1.WorkflowRunning, woc.wf.Status.Phase)
		pods, err := woc.controller.primaryProfile().kubernetesClient.CoreV1().Pods(wf.ObjectMeta.Namespace).List(ctx, metav1.ListOptions{})
		assert.NoError(t, err)
		assert.True(t, len(pods.Items) > 0, "pod was not created successfully")
...
func TestPDBCreation(t *testing.T) {
	...
	woc := newWorkflowOperationCtx(wf, controller)
	woc.operate(ctx)
	pdb, _ := controller.primaryProfile().kubernetesClient.PolicyV1beta1().PodDisruptionBudgets("").Get(ctx, woc.wf.Name, metav1.GetOptions{})
	assert.Equal(t, pdb.Name, wf.Name)
	woc.markWorkflowSuccess(ctx)
	_, err := controller.primaryProfile().kubernetesClient.PolicyV1beta1().PodDisruptionBudgets("").Get(ctx, woc.wf.Name, metav1.GetOptions{})
...
func TestPDBCreationRaceDelete(t *testing.T) {
	...
	woc.operate(ctx)
	makePodsPhase(ctx, woc, apiv1.PodSucceeded)
	err := controller.primaryProfile().kubernetesClient.PolicyV1beta1().PodDisruptionBudgets("").Delete(ctx, woc.wf.Name, metav1.DeleteOptions{})
	assert.NoError(t, err)
	woc = newWorkflowOperationCtx(woc.wf, controller)
	woc.operate(ctx)
	assert.Equal(t, wfv1.WorkflowSucceeded, woc.wf.Status.Phase)
...
func TestConfigMapCacheLoadOperate(t *testing.T) {
	...
	ctx := context.Background()
	_, err := controller.primaryProfile().workflowClient.ArgoprojV1alpha1().Workflows(wf.ObjectMeta.Namespace).Create(ctx, wf, metav1.CreateOptions{})
	assert.NoError(t, err)
...
func TestConfigMapCacheLoadOperateMaxAge(t *testing.T) {
	...
	_, err = controller.primaryProfile().kubernetesClient.CoreV1().ConfigMaps("default").Create(ctx, &nonExpiredEntry, metav1.CreateOptions{})
	assert.NoError(t, err)
	...
	_, err = controller.primaryProfile().kubernetesClient.CoreV1().ConfigMaps("default").Create(ctx, &expiredEntry, metav1.CreateOptions{})
	assert.NoError(t, err)
...
func TestStorageQuota(t *testing.T) {
	...
	controller.primaryProfile().kubernetesClient.(*fake.Clientset).BatchV1().(*batchfake.FakeBatchV1).Fake.PrependReactor("create", "persistentvolumeclaims", func(action k8stesting.Action) (bool, runtime.Object, error) {
