...
spec:
  tasks:
	...
	t.Run("CreateTaskSet", func(t *testing.T) {
		...
		woc.operate(ctx)
		tslist, err := woc.controller.primaryProfile().workflowClient.ArgoprojV1alpha1().WorkflowTaskSets("default").List(ctx, v1.ListOptions{})
		...
		pods, err := woc.controller.primaryProfile().kubernetesClient.CoreV1().Pods("default").List(ctx, v1.ListOptions{})
		assert.NoError(t, err)
...
status:
  nodes:
	...
	t.Run("RemoveCompletedTaskSetStatus", func(t *testing.T) {
		...
		defer cancel()
		_, err := controller.primaryProfile().workflowClient.ArgoprojV1alpha1().WorkflowTaskSets("default").Create(ctx, &ts, v1.CreateOptions{})
		assert.NoError(t, err)
		woc := newWorkflowOperationCtx(wf, controller)
...
status:
  nodes:
	...
	t.Run("MemoizeOnTaskSetSucceeded", func(t *testing.T) {
		...
		err = woc.reconcileTaskSet(ctx)
		assert.NoError(t, err)
		memo, err := controller.primaryProfile().kubernetesClient.CoreV1().ConfigMaps("default").Get(ctx, "cache-demo-1", v1.GetOptions{})
		assert.NoError(t, err)
