...
func TestWFLevelAutomountServiceAccountToken(t *testing.T) {
	woc := newWoc()
	ctx := context.Background()
	_, err := util.CreateServiceAccountWithToken(ctx, woc.controller.primaryProfile().kubernetesClient, "", "foo", "foo-token")
	assert.NoError(t, err)
...
func TestTmplLevelExecutorServiceAccountName(t *testing.T) {
	...
	_, err := util.CreateServiceAccountWithToken(ctx, woc.controller.primaryProfile().kubernetesClient, "", "foo", "foo-token")
	assert.NoError(t, err)
	_, err = util.CreateServiceAccountWithToken(ctx, woc.controller.primaryProfile().kubernetesClient, "", "tmpl", "tmpl-token")
	assert.NoError(t, err)
	...
	_, err = woc.executeContainer(ctx, woc.execWf.Spec.Entrypoint, tmplCtx.GetTemplateScope(), &woc.execWf.Spec.Templates[0], &wfv1.WorkflowStep{}, &executeTemplateOpts{})
	assert.NoError(t, err)
	pods, err := woc.controller.primaryProfile().kubernetesClient.CoreV1().Pods("").List(ctx, metav1.ListOptions{})
	assert.NoError(t, err)
	assert.Len(t, pods.Items, 1)
	pod := pods.Items[0]
	assert.Equal(t, "exec-sa-token", pod.Spec.Volumes[2].Name)
